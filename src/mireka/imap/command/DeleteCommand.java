...
public class DeleteCommand extends Command {
    ...
    public DeleteCommand(Session session, CommandParser parser) {
    ...
    public Completion execute() throws IOException, ImapException {
        ...
        if (!accessControl().hasAccess(mbname, Right.DELETE_MAILBOX)) {
            if ((accessControl().hasAccess(mbname, Right.LOOKUP)))
                throw new NoPermissionException(); // does not reveal secret information
            else
                throw new NonExistentException();
        }
        if (mailbox == session.mailbox)
            throw new CompletionException(ResponseCode.INUSE, "Selected mailbox cannot be deleted");
        UnilateralResponseOption option = new UnilateralResponseOption();
        option.notForSession = session;
        repository().removeMailbox(mbname, option);
